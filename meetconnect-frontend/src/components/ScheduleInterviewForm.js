...
const ScheduleInterviewForm = () => {
  ...
  return (
    <div className="schedule-form max-w-lg mx-auto p-6 bg-white shadow-md rounded-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <select name="type" onChange={handleChange} required className="border p-2 w-full">
        ...
        <input type="text" name="interviewer" onChange={handleChange} required className="border p-2 w-full" />

        {/* Resources Input Fields */}
        <div className="mt-4">
          <h3 className="text-lg font-semibold">Add Resources (Optional):</h3>

          <label className="block font-medium">Resource Title:</label>
          <input type="text" value={resourceTitle} onChange={(e) => setResourceTitle(e.target.value)} className="border p-2 w-full" />

          <label className="block font-medium">Resource Link:</label>
          <input type="url" value={resourceLink} onChange={(e) => setResourceLink(e.target.value)} className="border p-2 w-full" />

          <button type="button" onClick={addResource} className="bg-blue-500 text-white px-4 py-2 mt-2 rounded">
            Add Resource
          </button>
        </div>
          ...
          <ul className="mt-4 list-disc pl-5">
            {interviewData.resources.map((res, index) => (
              <li key={index} className="text-blue-500">
                {res.title} - <a href={res.link} target="_blank" rel="noopener noreferrer" className="underline">{res.link}</a>
              </li>
            ))}
          </ul>
        )}
